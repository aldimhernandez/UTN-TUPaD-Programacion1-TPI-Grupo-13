...
def guardar_paises(nombre_archivo, paises):
    ...
    )  # Abrimos el archivo en modo sobrescribir
    campos = [
        "nombre",
        "poblacion",
        "superficie",
        "continente",
    ]  # Definimos los headings en una lista
    escritor = csv.DictWriter(
        archivo, fieldnames=campos
    )  # creamos un escritor para paises.csv con los headings
...
def agregar_pais(paises):
    ...
    superficie = input(
        "Superficie (km2): "
    ).strip()  # Solicita ingresar superficie sin espacios
    continente_input = input(
        "Continente: "
    ).strip()  # Solicita ingresar continente sin espacios

    continente = encontrar_continente(
        continente_input
    )  # Convierte el continente ingresado en uno valido de CONTINENTES_VALIDOS[]

    if not nombre.replace(
        " ", ""
    ).isalpha():  # Sí no es una cadena de caracteres solo alfabéticos
        print(
            "❌ El nombre del país debe contener solo letras."
        )  # Muestra mensaje de error
        return  # Vuelve al menú
    if (
        not poblacion.isdigit() or int(poblacion) <= 0
    ):  # Sí no es un valor de solo dígitos numéricos positivo
        print(
    ...
    if continente is None:  # Sí es un valor vacío
        ...
        print(
            f"🌍 Continentes válidos: {', '.join(CONTINENTES_VALIDOS)}"
        )  # Muestra los continentes válidos
        return  # Vuelve al menú
    ...
    guardar_paises("src/paises.csv", paises)  # Actualiza paises.csv con el nuevo pais
    print(
        "✅ País agregado correctamente."
    )  # Informa al usuario que el país se guardo correctamente
...
def actualizar_pais(paises):  # Recibe una lista, en este caso de paises[]
    ...
    ).strip()  # Solicita el nombre sin espacios
    nombre_normalizado = normalizar(nombre_input)  # Lo normaliza

    # Por cada elemento pais en paises[]
    for pais in paises:
        if (
            normalizar(pais["nombre"]) == nombre_normalizado
        ):  # Sí encuentra el pais ingresado
            nueva_pob = input(
                "Nueva población: "
            ).strip()  # Solicita actualizar población
            nueva_sup = input(
                "Nueva superficie: "
            ).strip()  # Solicita actualizar superficie
            if (
                not nueva_pob.isdigit() or int(nueva_pob) <= 0
            ):  # Valida el valor para población
                print("❌ La población debe ser un número entero positivo.")
                return
            if (
                not nueva_sup.isdigit() or int(nueva_sup) <= 0
            ):  # Valida el valor para superficie
...
def buscar_pais(paises):
    ...
    resultados = [
        p for p in paises if termino_norm in normalizar(p["nombre"])
    ]  # Normaliza el resultado de búsqueda si coincide

    if resultados:  # Muestra los resultados
        print("\n🔎 Resultados encontrados:")
        for p in resultados:
            print(
                f"{p['nombre']} - Población: {p['poblacion']} - Superficie: {p['superficie']} km² - Continente: {p['continente']}"
            )
...
def filtrar_paises(paises):
    ...
    if opcion == "1":  # Por continente
        ...
        ).strip()  # Solicita continente sin espacios
        continente = encontrar_continente(
            continente_input
        )  # Obtiene el continente válido
        ...
        filtrados = [
            p for p in paises if p["continente"] == continente
        ]  # Guarda los paises del continente ingresado
        ...
        if min_pob > max_pob or min_pob < 0:  # Sí alguna entrada es menor que 0
            ...
            return  # Vuelve al menú principal
        filtrados = [
            p for p in paises if min_pob <= p["poblacion"] <= max_pob
        ]  # Si pasaron las validaciones guarda las coincidencias en filtrados[]
    ...
    if filtrados:
        ...
        for p in filtrados:
            print(
                f"{p['nombre']} - {p['continente']} - Población: {p['poblacion']} - Superficie: {p['superficie']} km²"
            )
    else:
...
def ordenar_paises(paises):
    ...
    orden = (
        input("Ascendente (A) o Descendente (D): ").strip().upper()
    )  # Solicita entrada para orden sin espacios y en mayúsculas

    if campo not in ["1", "2", "3"] or orden not in [
        "A",
        "D",
    ]:  # Sí las entradas son incorrectas
        print("❌ Opción inválida.")
        return  # Vuelve al menú principal

    reverse = (
        orden == "D"
    )  # Sí el orden en D reverse toma el valor de true y el orden sera descendiente, si el "A" = false ascendiente
    if campo == "1":
        ordenados = sorted(
            paises, key=obtener_nombre, reverse=reverse
        )  # Ordena por nombre usando la función auxiliar obtener_nombre
    elif campo == "2":
        ordenados = sorted(
            paises, key=obtener_poblacion, reverse=reverse
        )  # Ordena por población usando la función auxiliar obtener_poblacion
    elif campo == "3":
        ordenados = sorted(
            paises, key=obtener_superficie, reverse=reverse
        )  # Ordena por superficie usando la función auxiliar obtener_superficie

    print("\n📋 Países ordenados:")  # Muestra el resultado con el orden solicitado
    for p in ordenados:
        print(
            f"{p['nombre']} - Población: {p['poblacion']} - Superficie: {p['superficie']} km²"
        )  # Imprime cada país con su información formateada
...
def mostrar_estadisticas(paises):
    ...
    promedio_pob = sum(obtener_poblacion(p) for p in paises) / len(
        paises
    )  # Promedio de poblacion
    promedio_sup = sum(obtener_superficie(p) for p in paises) / len(
        paises
    ...
    for p in paises:  # Recorremos paises[]
        ...
        continentes[cont] = (
            continentes.get(cont, 0) + 1
        )  # Crea el continente como elemento e inicializa en 1 pero si ya existe incrementa en 1
...
def ejecutar_programa():
    # Llama a la función cargar_paises que almacena los paises en paises[] desde paises.csv
    paises = cargar_paises("src/paises.csv")
    while True:
        ...
        if opcion == "1":
            ...
            actualizar_pais(
                paises
            )  # Llama a la función para actualizar población y superficie de un pais existente
        elif opcion == "3":
            buscar_pais(paises)  # Llama a la función que busca un pais
        elif opcion == "4":
            filtrar_paises(
                paises
            )  # Llama a la función que filtra países por continente, rango de poblacion o superficie
        elif opcion == "5":
            ordenar_paises(
                paises
            )  # Llama a la función que ordena por nombre, poblacion o superficie
        elif opcion == "6":
            mostrar_estadisticas(
                paises
            )  # Llama a la función que muestra las estadisticas de los paises (mayor población, etc).
        elif opcion == "7":
            print("👋 ¡Hasta luego!")
